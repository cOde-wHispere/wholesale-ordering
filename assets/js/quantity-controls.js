(function () {
    ...
    function updateQuantity(input, direction) {
        ...
        if (!isFinite(current)) { current = isFinite(min) ? min : 0; }
    ...
    function addControls(container) {
        ...
        if (!input || container.querySelector('.wholesale-quantity-button')) { return; }
        ...
        decrease.addEventListener('click', function () { updateQuantity(input, -1); });
        increase.addEventListener('click', function () { updateQuantity(input, 1); });
        container.insertBefore(decrease, input);
        container.appendChild(increase);
    }

    function init() { document.querySelectorAll('.quantity').forEach(addControls); }
